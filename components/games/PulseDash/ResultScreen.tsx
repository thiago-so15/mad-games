...
export function ResultScreen({ distance, bestDistance, onRetry }: ResultScreenProps) {
  const isNewRecord = distance >= bestDistance && bestDistance > 0;
  return (
    <div className="flex flex-col items-center gap-6 animate-[fade-in_0.3s_ease-out]">
      <h2 className="text-2xl font-bold text-white">Game Over</h2>
      <p className="text-zinc-400">Distancia recorrida</p>
      <div className="rounded-xl border border-zinc-700 bg-zinc-900/80 px-8 py-6 text-center">
        <p className="text-3xl font-bold text-white">{Math.floor(distance)}</p>
        {bestDistance > 0 && <p className="mt-2 text-sm text-zinc-500">Mejor: <strong className="text-zinc-300">{Math.floor(bestDistance)}</strong></p>}
        {isNewRecord && distance > 0 && <p className="mt-1 text-sm font-medium text-amber-400">¡Nuevo récord!</p>}
      </div>
