...
export function ResultScreen({
  ...
  const isNewRecord = score >= bestScoreEver && bestScoreEver > 0;

  return (
    <div className="flex flex-col items-center gap-6 animate-[fade-in_0.3s_ease-out]">
      <h2 className="text-2xl font-bold text-white">Polaridad incorrecta</h2>
      <p className="text-zinc-400">El impacto no coincidió con tu polaridad</p>

      <div className="rounded-xl border border-zinc-700 bg-zinc-900/80 px-8 py-6 text-center">
        <p className="text-3xl font-bold text-white">{score}</p>
        <p className="mt-2 text-sm text-zinc-500">impactos superados</p>
        <p className="mt-1 text-sm text-zinc-500">
          Mejor racha: <strong className="text-zinc-300">{bestCombo}</strong>
        </p>
        {bestScoreEver > 0 && (
          <p className="mt-1 text-sm text-zinc-500">
            Mejor partida: <strong className="text-zinc-300">{bestScoreEver}</strong>
          </p>
        )}
        {isNewRecord && score > 0 && (
